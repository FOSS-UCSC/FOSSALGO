...
# C, CPP, JavaScript, Golang
REGEX_LOWER_UNDERSCORE="^[a-z]([a-z|_]?[a-z])+.(c|cpp|js|go)$"
...
for i in {1..3} ; do
  ...
  for d in `find $basePath/algorithms -mindepth $i -maxdepth $i` ; do
    ...
    if [ $i == 3 ] ; then
      ...
      if ([ ${arrPath[-2]} == "java" ] || [ ${arrPath[-2]} == "csharp" ]) && ! [[ ${arrPath[-1]} =~ $REGEX_UPPER_CAMEL ]]; then
        exitWithError "$d is not in a valid naming convention"
      fi

      if ([ ${arrPath[-2]} == "c" ] || [ ${arrPath[-2]} == "cpp" ] || [ ${arrPath[-2]} == "javascript" ] || [ ${arrPath[-2]} == "golang" ]) && ! [[ ${arrPath[-1]} =~ $REGEX_LOWER_UNDERSCORE ]]; then
        exitWithError "$d is not in a valid naming convention"
      fi
    fi
